...
public class CircularDupla<T> {
	...
	public No<T> getLast() throws IllegalArgumentException {
		if(this.ultimo_elemento == null)
			throw new IllegalArgumentException("Item nao existe");
	...
	public void remove(No<T> remover) {
		...
		if(this.ultimo_elemento == remover && anterior == remover 
		&& proximo == remover) {
	...
	public String toString() {
		...
		while(buffer.getProximo() != this.ultimo_elemento.getProximo()){
			builder.append(",");
			buffer = buffer.getProximo();
			builder.append(buffer.getValor());
		}
